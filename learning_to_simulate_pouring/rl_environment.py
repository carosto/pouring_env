...
class PouringEnv(gym.Env):
    ...
    def __init__(
        ...
        render_mode=None,
        target_particles_path="/home/carola/masterthesis/pouring_env/learning_to_simulate_pouring/particle_states/saved_particles_final_state.npz",
    ):
        super().__init__()
        ...
        self.pt_cup_wgt = 5.0
        self.pt_spill_wgt = -2
        self.pt_other_wgt = 1.0
        self.action_cost = -0.01
        self.jug_resting_wgt = -0.000001
        ...
        self.bounds_object_movement = (
            ...
            -5,
            5,
        )  # xmin, xmax, ymin, ymax, zmin, zmax (TODO: might need to be modified)
        self.max_abs_velocities = np.array(
            [5, 5, 5, 5, 5, 5]
        ...
        self._target_particles = jnp.load(target_particles_path)["arr_0"]
        # set up rendering (if required)
        self.render_mode = render_mode

        if self.render_mode == "rgb_array":
            self._canvas = SceneCanvas(
                size=(800, 600), keys=None, show=False, bgcolor="grey"
            )
    ...
    def reset(self, seed, options=None):
        ...
        if not self.model_loaded:
            ...
            self._fast_model_step = jax.jit(
                lambda p, g: self._apply_model_processing_step(p, g)
            )
        ...
        self.state_liq_pos_full_traj = jnp.zeros(
            (self.max_time, self.max_n_liq_node_per_graph, INPUT_SEQUENCE_LENGTH, 3)
        )
        self.state_mesh_node_pos_full_traj = jnp.zeros(
            (self.max_time, self.max_edges_m_per_graph, INPUT_SEQUENCE_LENGTH, 3)
        )
        self.state_mesh_pose_full_traj = jnp.zeros(
            (self.max_time, 3, INPUT_SEQUENCE_LENGTH, 6)
        )  # 3 objects
        ...
        input_graph = pouring_env.build_graph(
        ...
        self.idx_rim_jug, self.idx_rim_cup = self._identify_rim_jug_cup(
            self.state_mesh_node_pos_full_traj[0][:, -1], self.bb_cup
        )

        # IF REQUIRED: MOVE JUG TO DIFFERENT INITIAL POSITION
        if self.use_different_initial_position:
            input_graph, updated_data = self._move_to_different_initial_position(
                jug_pose, input_graph
            )
            self.state_liq_pos_full_traj = self.state_liq_pos_full_traj.at[0].set(
                updated_data["liq_position"][:-1]
            )
            self.state_mesh_node_pos_full_traj = self.state_mesh_node_pos_full_traj.at[
                0
            ].set(updated_data["mesh_position"])
            self.state_mesh_pose_full_traj = self.state_mesh_pose_full_traj.at[0].set(
                updated_data["mesh_pose"]
            )
            jug_pose = self.state_mesh_pose_full_traj[0][0, -1]

        # (_, dict_latent_graphs), _  = self.model.apply(self.network_params['params'], self.network_params['state'], None, input_graph)
        (output_graph, dict_latent_graphs), _ = self._fast_apply(
            self.network_params["params"], self.network_params["state"], input_graph
        )
        ...
        self._last_particles_cup = PouringEnv._get_particles_in_cup_smooth(
            predicted_liquid_pos, self.bb_cup, alpha=self.alpha
        )
        self._last_particles_spilled = PouringEnv._get_particles_on_floor_smooth(
            predicted_liquid_pos, z_floor=self.z_floor, alpha=self.alpha
        )
    ...
    def step(self, action):
        ...
        state_mesh_node_pos_cur = self.state_mesh_node_pos_full_traj[
            self.current_step_id - 1
        ]
        ...
        action_full = action_full.at[3].set(
            action[0]
        )  # currently only rotation around x-axis (TODO: extend)
        new_jug_pose, new_velocity = self.update_pose_single_step(
            prev_jug_pose, prev_jug_velocity, action_full
        )
        ...
        self.state_liq_pos_full_traj = self.state_liq_pos_full_traj.at[
            self.current_step_id
        ].set(out_data["liq_position"][:-1])
        self.state_mesh_node_pos_full_traj = self.state_mesh_node_pos_full_traj.at[
            self.current_step_id
        ].set(out_data["mesh_position"])
        self.state_mesh_pose_full_traj = self.state_mesh_pose_full_traj.at[
            self.current_step_id
        ].set(out_data["mesh_pose"])

        obs = self._get_observation(
            latent_liq_representation,
            new_jug_pose,
            new_velocity,
            particles=predicted_liquid_pos,
        ...
        reward = self._compute_reward(
            particles=predicted_liquid_pos,
            jug_pose=out_data["mesh_pose"][0, -1],
            velocity=self.current_jug_velocity,
            mesh_positions=self.state_mesh_node_pos_full_traj[self.current_step_id][
                :, -1
            ],
            exceeding_velocity_limit=exceeding_velocity_limit,
            action=action,
        )
        # print(self.jug_resting_wgt * out_data['mesh_pose'][0, -1][3]**2)
        terminated, truncated = self._is_done()

        if self.clear_cache_bool and (
            self.current_step_id % 500 == 0
        ):  # currently not needed anymore (after applying jiit to multiple functions)?
            jax.clear_caches()
    ...
    def _move_to_different_initial_position(self, initial_jug_pose, initial_graph):
        ...
        def clamp_control(u):
            return 1.0 * jax.nn.tanh(u)

        jug_pose_traj = pouring_env.create_jug_control_acc(
            initial_jug_pose, clamp_control(action_sequence)
        )
    ...
    def _identify_rim_jug_cup(self, mesh_positions, bbox_cup):
        ...
        cup_mask = (  # mask for positions in bounding box for cup
            (mesh_positions[:, 0] >= xmin)
            & (mesh_positions[:, 0] <= xmax)
            & (mesh_positions[:, 1] >= ymin)
            & (mesh_positions[:, 1] <= ymax)
            & (mesh_positions[:, 2] >= zmin)
            & (mesh_positions[:, 2] <= zmax)
        )
        jug_mask = ~cup_mask

        jug_positions = mesh_positions[
            jug_mask
        ]  # this technically also includes the stem of the martini glass
        cup_positions = mesh_positions[
            cup_mask
        ]  # this includes only the cup potion of the martini glass, not the stem

        # jug rim is jug point with highest z
        jug_z = jug_positions[:, 2]
        max_z_v = jnp.max(jug_z)
        jug_rim_idx = jnp.where(jug_z == max_z_v)[0][
            12
        ]  # jnp.argmax(jug_z) # HARDCODED BECAUSE FOR SOME REASON MULTIPLE POSITIONS HAD THE SAME HIGH Z VALUE (TODO: needs to be modified if jug changes)
    ...
    def _calc_distance_jug_cup(mesh_positions, idx_rim_jug, idx_rim_cup):
        return jnp.linalg.norm(
            mesh_positions[idx_rim_jug] - mesh_positions[idx_rim_cup]
        )

    def _render_frame_to_array(self):
        liquid_positions = np.array(
            self.state_liq_pos_full_traj[self.current_step_id - 1]
        )[:, -1]
        mesh_position = np.array(
            self.state_mesh_node_pos_full_traj[self.current_step_id - 1]
        )[:, -1]
        ...
        self._markers_liquid.set_data(
            liquid_positions, face_color=[0.1, 0.1, 0.1, 0.3], size=4
        )
    ...
    def _get_observation(
        ...
        velocity_normalized = (
            2
            * (jug_velocity - min_abs_velocity)
            / (self.max_abs_velocities - min_abs_velocity)
        ) - 1
        ...
        distance = PouringEnv._calc_distance_jug_cup(
            mesh_positions, self.idx_rim_jug, self.idx_rim_cup
        )
        ...
        p_count_cup, p_count_jug = self._get_particle_counts(
            particles, self.bb_cup, self.z_floor
        )
    ...
    def _get_particle_counts(particles, bb_cup, z_floor):
        ...
        count_jug = (
            len(particles) - count_cup - count_spilled
        )  # currently ignoring particles currently pouring
    ...
    def _compute_reward(
        ...
        reward, new_particles_cup, new_particles_spilled, curr_fill_level = (
            self._compute_reward_jax(
                ...
                self.pt_spill_wgt,
                self.action_cost,
                self.jug_resting_wgt,
                self.jug_velocity_wgt,
                self.idx_rim_jug,
                self.idx_rim_cup,
                self._target_particles,
                self._last_particles_cup,
                self._last_particles_spilled,
                self.target_fill_level,
            )
        )
    ...
    def _compute_reward_jax(
        ...
        distance = PouringEnv._calc_distance_jug_cup(
            mesh_positions, idx_rim_jug, idx_rim_cup
        )
        # TODO: these are currently only for the x rotation case!
        max_distance = 2.2347841
        min_distance = 0.26558006
        ...
        particles_cup = PouringEnv._get_particles_in_cup_smooth(
            particles, bb_cup, alpha=alpha
        )
        particles_spilled = PouringEnv._get_particles_on_floor_smooth(
            particles, z_floor=z_floor, alpha=alpha
        )
        particles_cup_diff = (particles_cup - last_particles_cup) / len(
            particles
        )  # normalize by number of particles
        particles_spilled_diff = (particles_spilled - last_particles_spilled) / len(
            particles
        )  # normalize by number of particles
        curr_fill_level = particles_cup / len(
            particles
        )  # percentage of particles in cup TODO: use actual fill level instead of just percentage of particles in cup
    ...
    @staticmethod
    @jax.jit
    def _get_particles_in_cup_smooth(particles, bb_cup, alpha=7.0):
    ...
    @staticmethod
    @jax.jit
    def _get_particles_on_floor_smooth(particles, z_floor, alpha=7.0):
    ...
    def _apply_model_processing_step(self, updated_jug_pose, input_graph):
        ...
        jug_nodes_curr = pouring_env.transform_mesh_to_local_coordinates(
            self.jug_vertices,
            updated_jug_pose[:3],
            ref_orientation=updated_jug_pose[3:],
        )
        curr_mesh_position = prev_mesh_position[0, :, -1]
        curr_mesh_position = curr_mesh_position.at[
        ...
        next_mesh_position = jnp.concatenate(
            [prev_mesh_position[0, :, 1:], curr_mesh_position[:, None]], axis=1
        )
        next_mesh_position_padded = prev_mesh_position.at[0].set(next_mesh_position)

        next_mesh_pose = jnp.concatenate(
            [prev_mesh_pose[0, :, 1:], curr_mesh_pose[:, None]], axis=1
        )
        ...
        next_pos_seq = jnp.concatenate(
            [prev_liq_position[:, 1:], pred_pos[:, None]], axis=1
        )

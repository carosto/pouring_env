...
def create_training_loop(
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % evaluate_every == 0:
            ...
            # Save the model as pickle
            save_path = f"{FLAGS.model_path}/epoch_{epoch+ 1}.pkl"
            save_model(params, state, save_path)   
...
def main(_):
    ...
    if FLAGS.mode in ['train', 'eval']:
        ...
        num_trials = 0
        print(f'Eval rollout on test set...: {FLAGS.data_path[0]}')
        if not os.path.exists(FLAGS.output_path):
            os.mkdir(FLAGS.output_path)
        with open(os.path.join(FLAGS.output_path, 'log.txt'), 'a') as logging_file:
            logging_file.write(f"Eval rollout on test set...: {FLAGS.data_path[0]}")
        #init model
       ...
       #for evaluation only pass onedataset at a time
        ...
        with open(os.path.join(FLAGS.output_path, 'log.txt'), 'a') as logging_file:
            logging_file.write(f"Loading latest model: {latest_model_path}")
        for fidx in range(len(file_list)):
            with h5py.File(os.path.join(f"{FLAGS.data_path[0]}/test", f'simulation_{fidx}.h5'), 'r') as hf:
                ...
                example_rollout, mse, chamfer_loss = rollout_evaluation.rollout(initial_graph, features, model, network_params)
                ...
                print(f"Trial {num_trials} (file: simulation_{fidx}.h5) - MSE: {mse}, chamfer: {chamfer_loss}")
                with open(os.path.join(FLAGS.output_path, 'log.txt'), 'a') as logging_file:
                    logging_file.write(f"\nTrial {num_trials} (file: simulation_{fidx}.h5) - MSE: {mse}, chamfer: {chamfer_loss}")
        average_mse = total_mse / num_trials
        average_chamfer_loss = total_chamfer_loss / num_trials
        print(f"\nAverage MSE across {num_trials} trials: {average_mse}")
        print(f"\nAverage chamfer_loss across {num_trials} trials: {average_chamfer_loss}")
        with open(os.path.join(FLAGS.output_path, 'log.txt'), 'a') as logging_file:
            logging_file.write(f"\nAverage MSE across {num_trials} trials: {average_mse}")
            logging_file.write(f"\nAverage chamfer_loss across {num_trials} trials: {average_chamfer_loss}")

...
class AccumulatedNormalizer(hk.Module):
  ...
  def update(self, data, mask):
    """Updates the normalizer with new data."""
    self._set_accumulator_shape(data)

    if mask is not None:
      data = jnp.where(mask[:, jnp.newaxis], data, jnp.zeros_like(data))
      count = jnp.sum(mask, dtype=jnp.float32) #dont take padded parts
    else:
      count = data.shape[0]

    hk.set_state('acc_sum', self._acc_sum + jnp.sum(data, axis=0))
    hk.set_state('acc_count', self._acc_count + count)
    hk.set_state('acc_sum_squared', self._acc_sum_squared + jnp.sum(data**2, axis=0))
...
class GraphElementsNormalizer(hk.Module):
  ...
  def normalize_node_array(self, name, array, mask=None):
    normalizer =  self.get_normalizer(name)
    normalised_data = normalizer(array)
    #update the normaliser with new data

    if mask is not None:
      normalizer.update(array, mask) #for mesh pose and node positions specify mask
    else:
      normalizer.update(array, self._node_mask)
